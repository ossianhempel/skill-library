...
describe("HTTP registry API", () => {
  ...
  it("serves health and catalog search", async () => {
    ...
    try {
      ...
      const catalog = await app.request(
        "/api/workspaces/workspace-1/packages?q=review"
      );
      ...
      await expect(catalog.json()).resolves.toEqual({
        packages: [
          expect.objectContaining({ id: "package-1", slug: "review-helper" }),
        ],
      });
    } finally {
      await store.close();
  ...
  it("omits non-approved packages from ordinary catalog search", async () => {
    ...
    try {
      ...
      const ordinary = await app.request(
        "/api/workspaces/workspace-1/packages"
      );
      ...
      await expect(maintainer.json()).resolves.toEqual({
        packages: expect.arrayContaining([
          ...
          expect.objectContaining({ id: "package-draft" }),
        ]),
      });
    } finally {
      await store.close();
  ...
  it("serves package details and latest approved versions", async () => {
    ...
    try {
      ...
      const version = await app.request(
        "/api/packages/package-1/latest-approved"
      );
      const usage = await app.request(
        "/api/workspaces/workspace-1/usage-counts?eventType=view&packageId=package-1",
        { headers: maintainerHeaders() }
      );
  ...
  it("serves package version lists and version detail", async () => {
    ...
    try {
      ...
      await expect(versions.json()).resolves.toEqual({
        versions: [
          expect.objectContaining({ id: "version-1", packageId: "package-1" }),
        ],
      });
  ...
  it("hides non-approved versions from ordinary version routes", async () => {
    ...
    try {
      ...
      const ordinaryVersions = await app.request(
        "/api/packages/package-1/versions"
      );
      const maintainerVersions = await app.request(
        "/api/packages/package-1/versions",
        { headers: maintainerHeaders() }
      );
      const deniedDetail = await app.request(
        "/api/versions/version-draft-list"
      );
  ...
  it("validates, ingests, and downloads artifacts", async () => {
    ...
    try {
      ...
      const download = await app.request(
        `/api/artifacts/${body.artifact.digest}/download?packageId=package-1&versionId=version-ingest`
      );
      const usage = await app.request(
        "/api/workspaces/workspace-1/usage-counts?eventType=download&packageId=package-1&versionId=version-ingest",
        { headers: maintainerHeaders() }
      );
  ...
  it("creates uploaded draft versions and transitions lifecycle state", async () => {
    ...
    try {
      const created = await app.request(
        "/api/workspaces/workspace-1/packages/upload",
        {
          method: "POST",
          headers: maintainerHeaders(),
          body: JSON.stringify({
      ...
      const published = await app.request(
        `/api/versions/${createdBody.version.id}/lifecycle`,
        {
          method: "POST",
          headers: maintainerHeaders(),
      ...
      await expect(published.json()).resolves.toEqual({
        version: expect.objectContaining({ lifecycleState: "published" }),
      });
  ...
  it("creates a default workspace when publishing into a fresh registry", async () => {
    ...
    try {
      const created = await app.request(
        "/api/workspaces/acme/packages/upload",
        {
          method: "POST",
          headers: maintainerHeaders(),
          body: JSON.stringify({
      ...
      const catalog = await app.request("/api/workspaces/acme/packages", {
        headers: maintainerHeaders(),
      });
  ...
  it("creates invalid draft versions but blocks approval", async () => {
    ...
    try {
      ...
      const approve = await app.request(
        `/api/versions/${body.version.id}/lifecycle`,
        {
          method: "POST",
          headers: maintainerHeaders(),
      ...
      await expect(approve.json()).resolves.toEqual({
        error: "Cannot approve a version with validation errors.",
      });
    } finally {
      await store.close();
    }
  ...
  it("returns frontmatter validation issues from the package-tree API", async () => {
    ...
    try {
      const response = await app.request("/api/validation/package-tree", {
        ...
        body: JSON.stringify({
          entries: [
            { path: "broken/SKILL.md", content: "# Missing frontmatter\n" },
          ],
        }),
      });
      const body = (await response.json()) as {
        validation: { ok: boolean; issues: { ruleId: string }[] };
      };

      expect(response.status).toBe(200);
      expect(body.validation.ok).toBe(false);
      expect(body.validation.issues).toContainEqual(
  ...
  it("creates invalid drafts for bad frontmatter but blocks approval", async () => {
    ...
    try {
      ...
      expect(body.version.validation.issues).toContainEqual(
        expect.objectContaining({ ruleId: "skill-md-missing-frontmatter" })
      );
      expect(approve.status).toBe(422);
    } finally {
  ...
  it("creates draft versions from Git imports with commit provenance", async () => {
    ...
    try {
      const response = await app.request(
        "/api/workspaces/workspace-1/packages/import-git",
        {
          method: "POST",
          headers: maintainerHeaders(),
          body: JSON.stringify({
  ...
  it("serves maintainer package and workspace reports with latest install state per install", async () => {
    ...
    try {
      ...
      const packageReport = await app.request(
        "/api/packages/package-1/report",
        { headers: maintainerHeaders() }
      );
      const workspaceReports = await app.request(
        "/api/workspaces/workspace-1/reports",
        { headers: maintainerHeaders() }
      );
      const denied = await app.request("/api/packages/package-1/report", {
        headers: userHeaders(),
      });
      ...
      await expect(workspaceReports.json()).resolves.toEqual({
        reports: [
          expect.objectContaining({
            packageId: "package-1",
            installs: expect.objectContaining({ total: 2 }),
          }),
        ],
      });
    } finally {
  ...
  it("serves catalog stats to viewers without exposing install reports", async () => {
    ...
    try {
      ...
      const catalogStats = await app.request(
        "/api/workspaces/workspace-1/catalog-stats",
        { headers: userHeaders() }
      );
      const reports = await app.request("/api/workspaces/workspace-1/reports", {
        headers: userHeaders(),
      });
      ...
      const upload = await app.request(
        "/api/workspaces/workspace-1/packages/upload",
        {
          ...
          body: JSON.stringify({
            ...
            version: "1.0.0",
            entries: [
              { path: "release-notes/SKILL.md", content: "# Release notes\n" },
            ],
          }),
        }
      );
  ...
  it("accepts configured bearer API keys for protected routes", async () => {
    ...
    process.env.SKILL_LIBRARY_API_KEYS =
      "maintainer-secret:maintainer:maintainer-api,user-secret:user:user-api";
    const { app, store } = await createSeededApp();

    try {
      const usage = await app.request(
        "/api/workspaces/workspace-1/usage-counts",
        {
            ...
            entries: [
              {
                ...
                content: skillMd("blocked", "Blocked skill example."),
              },
            ],
          }),
        }
  ...
  it("protects private workspace browse routes and lets admins update reporting policy", async () => {
    ...
    try {
      const deniedCatalog = await app.request(
        "/api/workspaces/private-workspace/packages"
      );
      const allowedCatalog = await app.request(
        "/api/workspaces/private-workspace/packages",
        { headers: userHeaders() }
      );
      const deniedDetail = await app.request("/api/packages/private-package");
      const deniedDownload = await app.request(
        "/api/artifacts/sha256:private/download?packageId=private-package&versionId=private-version"
      );
      const allowedDownload = await app.request(
        "/api/artifacts/sha256:private/download?packageId=private-package&versionId=private-version",
        {
          headers: userHeaders(),
        }
      ...
      const updated = await app.request("/api/workspaces/private-workspace", {
        ...
        body: JSON.stringify({
          reportingPolicy: "required",
          visibility: "public",
          logoUrl: "https://cdn.example.com/team-logo.svg",
        }),
      });
      const invalidLogo = await app.request(
        "/api/workspaces/private-workspace",
        {
          method: "PATCH",
          headers: adminHeaders(),
      ...
      const detail = await app.request("/api/workspaces/private-workspace");
      const publicCatalog = await app.request(
        "/api/workspaces/private-workspace/packages"
      );
  ...
  it("mints a personal agent token for signed-in users and accepts it for API auth", async () => {
    ...
    try {
      ...
      const minted = await app.request("/api/me/agent-token", {
        headers: userHeaders(),
      });
      ...
      const stable = await app.request("/api/me/agent-token", {
        headers: userHeaders(),
      });
  ...
  it("signed-in users can list team members with submission counts", async () => {
    ...
    try {
      ...
      await store
        .kysely!.insertInto("skill_versions")
        .values({
          id: "version-draft",
      ...
      const list = await app.request("/api/team/members", {
        headers: userHeaders(),
      });
      expect(list.status).toBe(200);
      const listBody = (await list.json()) as {
        members: Array<{ id: string; skillsSubmitted: number }>;
      };
  ...
  it("admin can list, update, and delete users", async () => {
    ...
    try {
      ...
      expect(list.status).toBe(200);
      const listBody = (await list.json()) as {
        users: Array<{ id: string; skillsSubmitted: number }>;
      };
      ...
      expect(updatedBody.user).toEqual(
        expect.objectContaining({ id: "user-1", role: "maintainer" })
      );
      ...
      const afterDelete = await app.request("/api/admin/users", {
        headers: adminHeaders(),
      });
...
async function createSeededApp(
  brandingOverrides: Partial<
    ReturnType<typeof defaultRegistryBrandingConfig>
  > = {}
) {
  const store = await createRegistryStore({ dataDir: await makeTmpDir() });
  await store.migrate();
...
async function createGitSkillRepo() {
  ...
  await writeFile(
    ...
    skillMd("git-skill", "Imported from Git.", "# Git skill\n")
  );
  await execFileAsync("git", ["init"], { cwd: repoPath, env: gitCommandEnv() });
  ...
  await execFileAsync("git", ["commit", "-m", "Add skill"], {
    cwd: repoPath,
    env: gitCommandEnv(),
  });
