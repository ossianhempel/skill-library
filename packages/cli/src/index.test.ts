...
describe("CLI install foundations", () => {
  it("resolves global and project install targets", async () => {
    ...
    expect(resolveInstallTarget({ target: "codex-global" })).toEqual(
      expect.objectContaining({ kind: "codex-global", root: "~/.codex/skills" })
    );
  ...
  it("reports missing, current, and stale metadata states", async () => {
    ...
    await writeFile(
      ...
      skillMd("demo-skill", "Demo skill.", "# Demo\n")
    );
    const contentDigest = validatePackageTree([
      {
        path: "SKILL.md",
        content: skillMd("demo-skill", "Demo skill.", "# Demo\n"),
      },
    ]).digest;

    await writeInstallMetadata(
      skillRoot,
      makeMetadata({ versionId: "version-1", contentDigest })
    );
    await expect(getInstalledSkillStatus(skillRoot, latest)).resolves.toEqual(
      expect.objectContaining({
        state: "stale",
    ...
    await writeInstallMetadata(
      skillRoot,
      makeMetadata({ versionId: "version-2", contentDigest })
    );
    await expect(getInstalledSkillStatus(skillRoot, latest)).resolves.toEqual(
      expect.objectContaining({
        state: "current",
  ...
  it("reports modified local content before update overwrite", async () => {
    ...
    await writeInstallMetadata(
      skillRoot,
      makeMetadata({ versionId: "version-1", contentDigest: "sha256:original" })
    );

    await expect(getInstalledSkillStatus(skillRoot, latest)).resolves.toEqual(
      expect.objectContaining({ state: "modified-local-content" })
    );
  });
  ...
  it("installs package files into a destination root and writes metadata", async () => {
    ...
    await expect(
      readFile(join(result.skillRoot, "SKILL.md"), "utf8")
    ).resolves.toBe(skillMd("demo-skill", "Demo skill.", "# Demo\n"));
  ...
  it("refuses unmanaged overwrites unless forced", async () => {
    ...
    const install = {
      ...
      entries: [
        {
          path: "demo-skill/SKILL.md",
          content: skillMd("demo-skill", "Remote skill.", "# Remote\n"),
        },
      ],
    };

    await expect(installPackageTree(install)).rejects.toThrow(
      "Refusing to overwrite unmanaged"
    );
    await expect(
      installPackageTree({ ...install, force: true })
    ).resolves.toEqual(expect.objectContaining({ skillRoot: unmanagedRoot }));
    await expect(
      readFile(join(unmanagedRoot, "SKILL.md"), "utf8")
    ).resolves.toBe(skillMd("demo-skill", "Remote skill.", "# Remote\n"));
  });
    ...
    const client = createRegistryClient({
      ...
      fetch: async (input, init) => {
        expect(String(input)).toBe(
          "http://registry.test/api/workspaces/workspace-1/packages?q=review"
        );
        expect(init).toEqual(
          expect.objectContaining({
        ...
        return jsonResponse({
          packages: [{ id: "package-1", slug: "review-helper" }],
        });
      },
    });

    await expect(client.search("workspace-1", "review")).resolves.toEqual([
      expect.objectContaining({ slug: "review-helper" }),
    ]);
  });
  ...
  it("installs the latest approved registry artifact and writes generated metadata", async () => {
    ...
    const entries = [
      {
        ...
        content: skillMd("remote-skill", "Remote skill.", "# Remote\n"),
      },
    ...
    const client = createRegistryClient({
      ...
      fetch: async (input, init) => {
        ...
        if (url.endsWith("/api/packages/package-1/latest-approved")) {
          return jsonResponse({
            version: makeVersion({ id: "version-2", artifactDigest: digest }),
          });
        }
    ...
    await expect(
      readFile(join(result.skillRoot, "SKILL.md"), "utf8")
    ).resolves.toBe(skillMd("remote-skill", "Remote skill.", "# Remote\n"));
    await expect(readInstallMetadata(result.skillRoot)).resolves.toEqual(
      expect.objectContaining({
        registryUrl: "http://registry.test",
        workspaceId: "workspace-1",
  ...
  it("honors workspace reporting policy during installs", async () => {
    ...
    expect(requiredReports).toEqual([
      expect.objectContaining({ state: "current", packageId: "package-1" }),
    ]);
  });
  ...
  it("updates stale managed installs to the latest approved version", async () => {
    ...
    const entries = [
      {
        path: "remote-skill/SKILL.md",
        content: skillMd("remote-skill", "Remote skill v2.", "# Remote v2\n"),
      },
    ];
    const digest = validatePackageTree(entries).digest!;
    const installedDigest = validatePackageTree([
    ...
    await writeFile(
      join(skillRoot, "SKILL.md"),
      skillMd("remote-skill", "Remote skill v1.", "# Remote v1\n")
    );
    await writeInstallMetadata(
      skillRoot,
      makeMetadata({
    ...
    await expect(readFile(join(skillRoot, "SKILL.md"), "utf8")).resolves.toBe(
      skillMd("remote-skill", "Remote skill v2.", "# Remote v2\n")
    );
    await expect(readInstallMetadata(skillRoot)).resolves.toEqual(
      expect.objectContaining({
        versionId: "version-2",
        contentDigest: installedDigest,
      })
  ...
  it("removes files deleted in newer managed updates", async () => {
    ...
    const initialEntries = [
      {
        path: "remote-skill/SKILL.md",
        content: skillMd("remote-skill", "Remote skill v1.", "# Remote v1\n"),
      },
      { path: "remote-skill/references/a.md", content: "A\n" },
    ];
    const entries = [
    ...
    await expect(
      stat(join(skillRoot, "references/a.md"))
  ...
  it("does not update current installs", async () => {
    ...
    const contentDigest = validatePackageTree([
      {
        path: "SKILL.md",
        content: skillMd("current-skill", "Current skill.", "# Current\n"),
      },
    ]).digest;
    const reports: unknown[] = [];
    ...
    await writeFile(
      join(skillRoot, "SKILL.md"),
      skillMd("current-skill", "Current skill.", "# Current\n")
    );
    await writeInstallMetadata(
      skillRoot,
      makeMetadata({
    ...
    await expect(
      updateFromRegistry({
        ...
        registryUrl: "http://registry.test",
        skillRoot,
        archivePath: join(await makeTmpDir(), "artifact.zip"),
      })
    ).resolves.toEqual(
      expect.objectContaining({
        updated: false,
    ...
    expect(reports).toEqual([
      expect.objectContaining({ state: "current", versionId: "version-1" }),
    ]);
  });
  ...
  it("runs search, info, install-plan, status, install, and update commands", async () => {
    ...
    const entries = [
      {
        ...
        content: skillMd("cli-skill", "CLI skill.", "# CLI\n"),
      },
    ];
    const digest = validatePackageTree(entries).digest!;
    const archive = await packPackageZip(entries);
    ...
    await expect(
      runCli(
        ["info", "package-1", "--registry", "http://registry.test"],
        runtime
      )
    ).resolves.toBe(0);
    await expect(
      runCli(["install-plan", "cli-skill", "--target", "project"], runtime)
    ).resolves.toBe(0);
    await expect(
      runCli(
        [
    ...
    await expect(
      runCli(
        [
          "status",
          "--root",
          join(destinationRoot, "cli-skill"),
          "--package",
          "package-1",
          "--registry",
          "http://registry.test",
    ...
    await expect(
      runCli(
        ["validate", "--root", join(destinationRoot, "cli-skill")],
        runtime
      )
    ).resolves.toBe(0);
    await expect(
    ...
    expect(
      outputs.some((line) =>
        line.includes(
          "npx @skill-library/cli install cli-skill --target project"
        )
      )
    ).toBe(true);
    await expect(
      readFile(join(destinationRoot, "cli-skill", "SKILL.md"), "utf8")
    ).resolves.toBe(skillMd("cli-skill", "CLI skill.", "# CLI\n"));
  });
  ...
  it("reports frontmatter validation issues from validate --root", async () => {
    ...
    await expect(
      runCli(["validate", "--root", skillRoot], {
...
async function installWithReportingPolicy(
  reportingPolicy: "disabled" | "required",
  ...
  const entries = [
    {
      path: "policy-skill/SKILL.md",
      content: skillMd("policy-skill", "Policy skill.", "# Policy\n"),
    },
  ];
  const digest = validatePackageTree(entries).digest!;
  const archive = await packPackageZip(entries);
